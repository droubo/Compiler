...
expr : assignexpr {fprintf(yyout,"expr -> assignexpr\n");}
     | expr op expr {if(flag_func == 1 && flag_op == 0) fprintf(yyout,"ERROR @ line %d: Unable to do this operation with function\n", yylineno); flag_op = 0; flag_func = 0; fprintf(yyout,"expr -> expr op expr\n");}
...
op : PLUS {fprintf(yyout,"op -> +\n");}
   ...
   | EQUAL {flag_op = 1; fprintf(yyout,"op -> ==\n");}
   | NOT_EQUAL {flag_op = 1; fprintf(yyout,"op -> !=\n");}
   | AND {flag_op = 1; fprintf(yyout,"op -> and\n");}
   | OR {flag_op = 1; fprintf(yyout,"op -> or\n");}
   ;

term : LEFT_PARENTHESIS expr RIGHT_PARENTHESIS {fprintf(yyout,"term -> ( expr )\n");}
     | MINUS expr {if(flag_func == 1) fprintf(yyout,"ERROR @ line %d: Unable to do this operation with function\n", yylineno); flag_func = 0; fprintf(yyout,"term -> - expr\n");}
     | NOT expr {fprintf(yyout,"term -> not expr\n");}
     | PLUS_PLUS lvalue {if(flag_func == 1) fprintf(yyout,"ERROR @ line %d: Unable to do this operation with function\n", yylineno); flag_func = 0; fprintf(yyout,"term -> ++ lvalue\n");}
     | lvalue PLUS_PLUS {if(flag_func == 1) fprintf(yyout,"ERROR @ line %d: Unable to do this operation with function\n", yylineno); flag_func = 0; fprintf(yyout,"term -> lvalue ++\n");}
     | MINUS_MINUS lvalue {if(flag_func == 1) fprintf(yyout,"ERROR @ line %d: Unable to do this operation with function\n", yylineno); flag_func = 0; fprintf(yyout,"term -- lvalue\n");}
     | lvalue MINUS_MINUS {if(flag_func == 1) fprintf(yyout,"ERROR @ line %d: Unable to do this operation with function\n", yylineno); flag_func = 0; fprintf(yyout,"lavlue --\n");}
